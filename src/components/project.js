...
class Project extends React.Component {
  ...
  handleDims = () => {
    ...
    const thumb = this.checkVar(
      () => document.querySelector('.photo-grid .slide-wrap'),
      false
    )
    if (thumb) {
  ...
  render() {
    ...
    const title = this.checkVar(
      () => this.state.data.exLink.title,
      `Visit Website`
    )
    const exLink = link ? (
    ...
    return (
      <div className='project-wrap-outer'>
        <Link to='/#portfolio' className='project-back-link'></Link>
        ...
        <div className='project-wrap-inner'>
              ...
              <div className='photo-grid-inner'>
                {this.state.slides.length > 0 && (
                  <div
                    ...
                    className='slide-wrap feat-image'
                  >
                    {this.state.slides[0].gif ? (
                      <div>
                        <img
                          src={this.state.slides[0].src}
                          alt={this.state.slides[0].title}
                        />
                      </div>
                    ) : (
                      this.state.slides[0] && (
                        <Image
                          fluid={this.state.slides[0].fluid}
                          alt={this.state.slides[0].title}
                        />
                      )
                    )}
          ...
          <div className='project-right'>
            <div className='project-right-inner'>
              <div className='project-head'>
                <h1>
                  <span className='project-type'>
                    {this.state.data.projectType.title}
                  </span>{' '}
                  <span className='project-name'>{this.state.data.title}</span>
                </h1>
                <p className='project-client'>
                  <label>Client</label> {this.state.data.client.title}
                </p>
                <ul className='project-tags'>
                  {this.state.data.tags.length > 0 &&
                    this.state.data.tags.map((elem, i) => {
                      return (
                        <li key={elem._id} className='project-tag'>{`${
                          elem.title
                        }${
                          i < this.state.data.tags.length - 1 ? `, ` : ``
                        }`}</li>
                      )
                    })}
                </ul>
              ...
              <div className='project-body-wrap'>{body()}</div>
                ...
                <div className='social-inner-wrap'>
                  {this.state.socialLinks.github && (
                    <div className='social-icon git-icon'>
                      <a
                        href={this.state.socialLinks.github}
                  ...
                  {this.state.socialLinks.dribbble && (
                    <div className='social-icon drib-icon'>
                      <a
                        href={this.state.socialLinks.dribbble}
                        rel='noopener noreferrer'
                        target='_blank'
                        title='Dribbble Post'
                        className='social-link'
                  ...
                  {this.state.socialLinks.behance && (
                    <div className='social-icon be-icon'>
                      <a
                        href={this.state.socialLinks.behance}
                        rel='noopener noreferrer'
                        target='_blank'
                        title='Behance Post'
                        className='social-link'
              ...
              <div className='tech'>
                ...
                <ul className='tech-list'>
                  {this.state.data.tech.length > 0 &&
                    this.state.data.tech.map((elem, i) => {
                      return (
                        <li className='tech-list-item'>{`${elem.title}${
                          i < this.state.data.tech.length - 1 ? `, ` : ``
                        }`}</li>
                      )
                    })}
                </ul>
